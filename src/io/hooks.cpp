	...
	ForEach(Att, 1, MaxAtt)
	{
		...
		if ( ! Dv )
		{
			...
			if ( Train )
			{
			    ...
			    if ( MaxAttVal[Att] >= (int) AttValName[Att][0] )
			    {
...
/*    ------  */
{
    ...
    R->Vote = (int)(1000 * (R->Correct + 1) / (R->Cover + 2) + 0.5);
...
/*   --------  */
{
    ...
    {
	FindLeaf(CaseDesc, T->Branch[0], PT, Fraction);
	return;
    }
...
ClassNo BoostClassify(Description CaseDesc, int MaxTrial)
/*	-------------  */
{
    ClassNo	c, Best;
    int		t;
    ...
    {
	Best = ( RULES ? RuleClassify(CaseDesc, RuleSet[t]) :
    ...
    return SelectClass(Default, (Boolean)(MCost != Nil));
}
